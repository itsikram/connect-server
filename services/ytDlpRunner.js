...
const nodeMajor =
  parseInt(String(process.versions.node || "0").split(".")[0], 10) || 0;
...
const jsonCookiesToNetscape = (cookies) => {
  ...
  for (const c of cookies) {
    ...
    const expiry = Math.floor(
      c.expirationDate || c.expires || Date.now() / 1000 + 86400 * 30,
    );
    lines.push(
      [domain, flag, cookiePath, secure, expiry, c.name, String(c.value)].join(
        "\t",
      ),
    );
  }
 ...
 */
const buildFormat = (height, mode = "preferred", audioOnly = false) => {
  if (audioOnly) {
...
const getJsRuntimeArgs = () => {
  ...
  if (process.env.YT_DLP_JS_RUNTIME) {
    ...
    fs.existsSync("/usr/local/bin/deno") ||
    fs.existsSync("/usr/bin/deno")
  ) {
    const deno = fs.existsSync("/usr/local/bin/deno")
      ? "/usr/local/bin/deno"
      : "/usr/bin/deno";
...
const isRetryableDownloadError = (message) => {
  if (isBotBlockError(message) || isFormatUnavailableError(message))
    return true;
  const lower = String(message || "").toLowerCase();
  return (
...
const resolveFfmpegLocation = () => {
  ...
  for (const c of candidates) {
    if (fs.existsSync(c)) {
      return c.endsWith("ffmpeg") || c.endsWith("ffmpeg.exe")
        ? path.dirname(c)
        : c;
    }
  }
...
const buildSpawnArgs = (
  url,
  { outputPath, height, playerClient, formatMode, useCookies, audioOnly },
) => {
  const args = [
    "--no-playlist",
    "--no-simulate",
    ...
    "--extractor-args",
    buildExtractorArgs(playerClient),
    "--retries",
    "5",
    "--fragment-retries",
    "5",
  ...
  if (audioOnly) {
    args.push(
      "-f",
      buildFormat(height, formatMode, true),
      "-x",
      "--audio-format",
      "mp3",
      "--audio-quality",
...
const isYtDlpAvailable = async () =>
  isStandaloneAvailable() || Boolean(youtubedl);
...
const findDownloadedFile = (outputDir, prefix) => {
  ...
  const files = fs
    ...
    .filter((f) => /\.(mp4|mkv|webm|m4a|opus|mp3)$/i.test(f))
    .map((f) => ({
      name: f,
...
const listDirDebug = (outputDir, prefix) => {
  try {
    ...
    const names = fs
      .readdirSync(outputDir)
      .filter((f) => f.includes(prefix) || f.startsWith(prefix));
...
const titleFromFilename = (filename, prefix) =>
  filename
    ...
    .replace(/\.(mp4|mkv|webm|m4a|opus|mp3)$/i, "")
    .replace(/_/g, " ")
    .trim() || "video";
...
const extractYtDlpError = (stderr, code, outputDir, outputPrefix) => {
  ...
  const useful = String(stderr || "")
    ...
    .filter(
      (l) =>
        l && !/^\[download\]/i.test(l) && !/%/.test(l) && !/^WARNING:/i.test(l),
    );
...
const resolveOutput = ({ outputDir, outputPrefix, stderr, stdout }) => {
  ...
  for (const candidate of [fromStderr, fromDir].filter(Boolean)) {
    if (fs.existsSync(candidate) && fs.statSync(candidate).size > 0) {
      const fromName = titleFromFilename(
        path.basename(candidate),
        outputPrefix,
      );
      const title =
        (printedTitle &&
        printedTitle.length > 2 &&
        !printedTitle.includes(outputPrefix)
          ? printedTitle
          : null) || fromName;
...
const downloadWithStandalone = ({
  ...
  new Promise((resolve, reject) => {
    ...
    proc.stderr.on("data", (chunk) => {
      ...
      text.split("\n").forEach((line) => {
        if (
          /javascript runtime|ejs|no video formats|sign in|ERROR:|Destination:|Merging|Downloading/i.test(
            line,
          )
        ) {
    ...
    proc.on("close", (code) => {
      ...
      if (resolved) {
        if (code !== 0) {
          console.warn(
            `[yt-download] yt-dlp exit ${code} but file exists (${path.basename(resolved.filePath)})`,
          );
        }
        resolve(resolved);
        return;
      ...
      console.warn(
        `[yt-download] missing file after exit ${code}. stderrTail=${stderr.trim().slice(-400)}`,
      );
      reject(
        new Error(extractYtDlpError(stderr, code, outputDir, outputPrefix)),
      );
    });
  });

const downloadWithExec = ({
  url,
  outputDir,
  outputPrefix,
  height,
  ...
  new Promise((resolve, reject) => {
    ...
    if (audioOnly) {
      flags.format = buildFormat(height, formatMode, true);
    ...
    for (let i = 0; i < jsArgs.length; i += 2) {
      ...
      if (jsArgs[i] === "--remote-components")
        flags.remoteComponents = jsArgs[i + 1];
    }
    ...
    if (subprocess.stderr) {
      subprocess.stderr.on("data", (chunk) => {
        ...
        text.split("\n").forEach((line) => {
          if (
            /javascript runtime|ejs|no video formats|sign in|ERROR:|Destination:|Merging/i.test(
              line,
            )
          ) {
            console.warn("[yt-download]", line.trim().slice(0, 240));
    ...
    const finish = () =>
      resolveOutput({ outputDir, outputPrefix, stderr, stdout });
    ...
    subprocess
      .then(() => {
        ...
        reject(
          new Error(extractYtDlpError(stderr, 0, outputDir, outputPrefix)),
        );
      })
      .catch((err) => {
        const ok = finish();
        ...
        reject(
          new Error(
            err?.message ||
              extractYtDlpError(stderr, 1, outputDir, outputPrefix),
          ),
        );
      });
  });
...
const downloadWithYtDlp = async ({
  ...
  console.log(
    `[yt-download] JS runtime args: ${getJsRuntimeArgs().join(" ") || "none"}`,
  );

  for (const attempt of DOWNLOAD_ATTEMPTS) {
    if (!audioOnly && attempt.formatMode === "preferred" && !height) continue;
    ...
    if (attempt.localOnly && (process.env.RENDER === "true" || hasCookies()))
      continue;
    ...
    try {
      ...
      lastError = err;
      console.warn(
        `[yt-download] player_client=${attempt.client} failed:`,
