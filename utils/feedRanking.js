const scorePost = (post, { connectIds = new Set(), currentUserId } = {}) => {
  ...
  const connectBoost = connectIds.has(authorId) ? 12 : 0;
  const selfBoost = String(currentUserId) === authorId ? 4 : 0;
  const officialBoost = post?.author?.isOfficial ? 6 : 0;
  return recency + reacts + comments + connectBoost + selfBoost + officialBoost;
};
